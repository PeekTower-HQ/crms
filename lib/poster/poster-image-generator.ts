...
/**
 * Fetch and process photo for poster
 */
async function fetchPhoto(
  ...
  try {
    ...
    return sharp(buffer)
      .resize(width, height, {
        fit: "cover",
        position: "attention", // Focus on faces
      })
      .png()
      .toBuffer();
  } catch (error) {
...
/**
 * Generate Wanted Person poster image
 */
export async function generateWantedPosterImage(
  ...
  const detailsSvg = generateDetailsPanelSvg(
    details,
    "#7C3AED", // Purple for label background
    "#FEF3C7"  // Cream for value background
  );
...
/**
 * Generate Amber Alert poster image
 */
export async function generateAmberAlertPosterImage(
  ...
  const bannerSvg = generateBannerSvg(
    "AMBER ALERT",
    AMBER_COLORS.primary,
    AMBER_COLORS.text
  );

  const missingDurationSvg =
    data.daysMissing !== null
      ? generateMissingDurationSvg(data.daysMissing)
      : null;
  ...
  const detailsSvg = generateDetailsPanelSvg(
    details,
    "#DC2626", // Red for label background (urgency)
    AMBER_COLORS.secondary
  );

  const descriptionSvg = generateTextSectionSvg(
    "DESCRIPTION",
    data.description,
    "#F9FAFB"
  );
