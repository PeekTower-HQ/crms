...
import { PosterColorTheme, PosterBranding } from "./poster-types";
import { getImageDimensions, POSTER_LAYOUT } from "./poster-config";
...
/**
 * Generate main banner SVG (WANTED / AMBER ALERT)
 */
...
/**
 * Generate missing duration bar SVG (for Amber Alerts)
 */
...
/**
 * Generate text section SVG (charges, description, etc.)
 */
...
/**
 * Generate photo placeholder SVG
 */
export function generatePhotoPlaceholderSvg(width: number, height: number): string {
  return `
