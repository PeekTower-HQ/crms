...
export class CaseService {
  ...
  private validateCaseInput(data: Partial<CreateCaseInput>): void {
    ...
    if (data.category) {
      // Use deployment configuration for offense categories
      const validCategories = this.countryConfig.getOffenseCategories();
      const categoryNames = validCategories.map(cat => cat.name);
      
      // Also collect subcategories
      const allSubcategories: string[] = [];
      validCategories.forEach(cat => {
        cat.subcategories.forEach((sub: string | { name: string }) => {
          allSubcategories.push(typeof sub === "string" ? sub : sub.name);
        });
      });
      
      // Allow both category names and subcategory names
      const allValid = [...categoryNames, ...allSubcategories, "other"];
      
      if (!allValid.includes(data.category)) {
        throw new ValidationError(
          `Invalid case category. Must be one of: ${allValid.join(", ")}`
        );
      }
    }
