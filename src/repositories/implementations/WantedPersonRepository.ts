  ...
  implements IWantedPersonRepository
{
  ...
  private toDomain(data: any): WantedPerson {
    return new WantedPerson(
      ...
      data.reward ? Number(data.reward) : null,
      data.contactPhone || "",
      data.regionalAlert, // Prisma uses 'regionalAlert' not 'isRegionalAlert'
      data.createdById,
    ...
    return this.execute(async () => {
      const created = await this.prisma.wantedPerson.create({
        data: {
          ...
          photoSmallUrl: data.photoSmallUrl || null,
          photoMediumUrl: data.photoMediumUrl || null,
    ...
    return this.execute(async () => {
      const updated = await this.prisma.wantedPerson.update({
        ...
        data: {
          ...
          ...(data.photoThumbnailUrl !== undefined && { photoThumbnailUrl: data.photoThumbnailUrl }),
          ...(data.photoSmallUrl !== undefined && { photoSmallUrl: data.photoSmallUrl }),
          ...(data.photoMediumUrl !== undefined && { photoMediumUrl: data.photoMediumUrl }),
          ...(data.photoHash !== undefined && { photoHash: data.photoHash }),
          ...(data.photoSize !== undefined && { photoSize: data.photoSize }),
          ...(data.photoUploadedAt !== undefined && { photoUploadedAt: data.photoUploadedAt }),
